...
interface AnnualContainerStatsResult {
  status: 200
  body: {
...
interface ContainerStatsResult {
  status: 200
  body: {
...
export function statsController(router: Router): void {
  router.get(
    ...
    async (ctx: Context): Promise<OverallStatsResult> => {
      ...
      const overall = await statsService.getOverall(
        async (statsFilter: StatsIdFilter) =>
  ...
  router.get(
    ...
    async (ctx: Context): Promise<AnnualContainerStatsResult> => {
      const authTokenPayload = parseAuthToken(ctx)
      const { skip, size } = ctx.request.query
      const statsFilter = validateStatsIdFilter(ctx.request.query)
      const pagination = validatePagination({ skip, size })
      const annualContainer = await statsService.getAnnualContainer(
        async (pagination: Pagination, statsFilter: StatsIdFilter) =>
  ...
  router.get(
    ...
    async (ctx: Context): Promise<BreweryStatsResult> => {
      const authTokenPayload = parseAuthToken(ctx)
      const { skip, size } = ctx.request.query
  ...
  router.get(
    ...
    async (ctx: Context): Promise<ContainerStatsResult> => {
      const authTokenPayload = parseAuthToken(ctx)
      const statsFilter = validateStatsIdFilter(ctx.request.query)
  ...
  router.get(
    ...
    async (ctx: Context): Promise<LocationStatsResult> => {
      const authTokenPayload = parseAuthToken(ctx)
      const { skip, size } = ctx.request.query
      const { order, direction } = ctx.request.query
      const pagination = validatePagination({ skip, size })
      const statsFilter = validateStatsFilter(ctx.request.query)
      const locationStatsOrder = validateLocationStatsOrder({
        order,
        direction,
      })
  ...
  router.get(
    '/api/v1/stats/rating',
    async (ctx: Context): Promise<RatingStatsResult> => {
      const authTokenPayload = parseAuthToken(ctx)
      const statsFilter = validateStatsIdFilter(ctx.request.query)
  ...
  router.get(
    '/api/v1/stats/style',
    async (ctx: Context): Promise<StyleStatsResult> => {
      const authTokenPayload = parseAuthToken(ctx)
      const { order, direction } = ctx.request.query
      const styleStatsOrder = validateStyleStatsOrder({ order, direction })
      const statsFilter = validateStatsFilter(ctx.request.query)
      const style = await statsService.getStyle(
        async (statsFilter: StatsFilter, styleStatsOrder: StyleStatsOrder) =>
