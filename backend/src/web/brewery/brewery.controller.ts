...
export function breweryController(router: Router): void {
  router.post('/api/v1/brewery', async (ctx: Context) => {
  ...
  router.put('/api/v1/brewery/:breweryId', async (ctx: Context) => {
    const authTokenPayload = authHelper.parseAuthToken(ctx)
    const body: unknown = ctx.request.body
  ...
  router.get('/api/v1/brewery/:breweryId', async (ctx: Context) => {
    const authTokenPayload = authHelper.parseAuthToken(ctx)
    const breweryId: string | undefined = ctx.params.breweryId
  ...
  router.get('/api/v1/brewery', async (ctx: Context) => {
    const authTokenPayload = authHelper.parseAuthToken(ctx)
  ...
  router.post('/api/v1/brewery/search', async (ctx: Context) => {
    const authTokenPayload = authHelper.parseAuthToken(ctx)
    const body: unknown = ctx.request.body
