...
import { type CreateContainerRequest, type UpdateContainerRequest, validateCreateContainerRequest, validateUpdateContainerRequest } from './container'
...
export function containerController (router: Router): void {
  router.post('/api/v1/container',
    ...
    async (ctx) => {
      ...
      const result = await ctx.db.executeTransaction(async (trx) => {
        return await containerService.createContainer(trx, createContainerRequest)
      })
  ...
  router.put('/api/v1/container/:containerId',
    ...
    async (ctx) => {
      ...
      const result = await ctx.db.executeTransaction(async (trx) => {
        return await containerService.updateContainer(trx, containerId, updateContainerRequest)
      })
...
function validateUpdateRequest (body: unknown, containerId: string): UpdateContainerRequest {
  if (!validateUpdateContainerRequest(body)) {
    throw new ControllerError(400, 'InvalidContainer', 'invalid container')
  }
  if (containerId === undefined || containerId === null || containerId.length === 0) {
