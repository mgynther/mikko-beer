...
import type { Beer, BeerList, BeerWithIds } from './beer'
...
test('validateBeerListOrUndefined returns undefined for undefined', () => {
  expect(validateBeerListOrUndefined(undefined)).toEqual(undefined)
})
...
test('validateBeerListOrUndefined throws for invalid input', () => {
  expect(() => validateBeerListOrUndefined({ beers: 'wrong' })).toThrow()
})
