...
test('parse YearMonth', () => {
  expect(
    parseYearMonth('2019-05', { year: 2016, month: 2 })
  ).toEqual({ year: 2019, month: 5})
})
...
parseYearMonthFallbackTests.forEach(testCase =>
  test(`fallback to default on parsing ${testCase.label} YearMonth`, () => {
    expect(
      parseYearMonth(testCase.value, { year: 2016, month: 2 })
    ).toEqual({ year: 2016, month: 2})
  })
)
