...
test('Add review', async ({ page }) => {
  ...
  await page.getByRole('combobox', { name: /search brewery/i }).fill('a.lec')
  await page.getByRole('option', { name: /a.lecoq/i }).click()
  await page.getByRole('combobox', { name: /search style/i }).fill('lag')
  await page.getByRole('option', { name: /^lager$/i }).click()
